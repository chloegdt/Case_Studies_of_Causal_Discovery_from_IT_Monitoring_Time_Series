...
def dynotears(data, tau_max=1, sig_level=0.05):
    sm = from_pandas_dynamic(data, p=tau_max, w_threshold=0.01, lambda_w=sig_level, lambda_a=sig_level)
    ...
    for tname in sm.nodes:
        x = tname.split("_lag")
        tname_to_name_dict[tname] = x[0]

    print(tname_to_name_dict)
    for ce in sm.edges:
        c = ce[0]
        e = ce[1]
        summary_matrix[tname_to_name_dict[e]].loc[tname_to_name_dict[c]] = 1
